...
class Masker(object):
    def __init__(
        ...
        self.device = device
        # self.coeff = torch.load('/home/vmoat/userstudy_wsbnew/contrastive_learn_fit3_dict_state_norm5_all_epoch32.pth')
        root_path = '/mnt/data/old_home/strength/vmoat'
        self.coeff = torch.load(root_path + '/userstudy_wsbnew/contrastive_learn_fit3_dict_state_norm5_all_epoch32.pth')
    ...
    def _compute_masking_threshold(self, x: np.ndarray, mode: str="vmoat") -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(psd.shape[1]):
            ...
            # print('【【【self.coeff】', type(self.coeff['a']))
            # 【【【self.coeff】 tensor([1.8110, 1.8081, 1.9901,  ..., 0.2887, 0.3003, 0.2611], device='cuda:0')
            # 【【【self.coeff】 OrderedDict([('a', tensor([1.8110, 1.8081, 1.9901,  ..., 0.2887, 0.3003, 0.2611], device='cuda:0')), ('b', tensor([2.2045, 1.5934, 2.2062,  ..., 0.0030, 0.0042, 0.0059], device='cuda:0'))])       

        theta_array = np.array(theta) + EPS       # 这里也就是论文qin公式（27）

        theta_array = self.coeff['a'].cpu().numpy() * theta_array + self.coeff['b'].cpu().numpy()     # theta = a * tmep  + b
