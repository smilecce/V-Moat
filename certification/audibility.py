...
class Audibility:
    ...
    def audibility(self, wav_delta: torch.Tensor, wav_x: torch.Tensor) -> np.ndarray:
        ...
        theta = torch.FloatTensor(theta).to(self.device) #  【2024-1-21改】      
        # theta = torch.FloatTensor(theta).to(self.device) + EPS      # avoid zeros 【2021-1-21 ori】
        # theta = self.coeff['a'] * theta + self.coeff['b']     # theta = a * tmep  + b
